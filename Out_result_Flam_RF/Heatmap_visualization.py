...
# 文件路径列表
file_paths = [
    'SHAP_values_train_Amine_F_re.csv',
    'SHAP_values_train_Aromatic_F_re.csv',
    'SHAP_values_train_Ether_F_re.csv',
    'SHAP_values_train_Halogen_F_re.csv',
    'SHAP_values_train_Hydroxyl_F_re.csv',
    'SHAP_values_train_Ketone_Aldehyde_F_re.csv',
    'SHAP_values_train_Nitrile_F_re.csv',
    'SHAP_values_train_Other_F_re.csv',
    'SHAP_values_train_Thioether_F_re.csv',
    'SHAP_values_train_Thiol_F_re.csv'
]
...
# 保存前四个特征到Excel
top_features_excel_path = os.path.join(output_dir, 'Top_4_Features_Per_Category.xlsx')
...
for label, data in shap_data.items():
    ...
    plt.ylabel('Mean |SHAP Value|')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'SHAP_Feature_Importance_{label}_4.png'))
    plt.close()
