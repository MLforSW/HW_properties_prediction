...
#
# # 加载数据
# file_path = 'imputed_selected_features_Flam.csv'
...
# results_df.to_excel('model_comparison_results_ml_flammability.xlsx', index=False)
...
# })
# importance_df.to_excel('sorted_feature_importances_flam.xlsx', index=False)
...
# plt.title('Feature Importance for flammability in RF ', fontsize=16, fontweight='bold')
#
# # 调整Y轴标签，放大字体并旋转标签
...
#
# # 先保存图形
# plt.savefig("feature_importance_in_XGBoost_flammability.png", bbox_inches='tight')
# import shap
# # 使用SHAP解释训练集上的模型预测
...
#
# # 确保输出目录存在
# output_dir = 'Out_result_Flam_RF'  # 更新为你想保存的路径
# os.makedirs(output_dir, exist_ok=True)
#
# # 初始化一个空列表来存储所有类别的 SHAP 值
# all_shap_values = []
#
# # 为训练集中每个分类计算SHAP值并存储
...
#
#     # 假设你只关心正类 SHAP 值，选择第一个元素
#     class_index = 1  # 根据你的实际分类情况，选择正确的类别索引
#     shap_values_class = shap_values_class[class_index]
#
#     # 将当前类别的 SHAP 值存储到列表中
#     all_shap_values.append(shap_values_class)
#
# # 将所有类别的 SHAP 值合并为一个numpy数组
# combined_shap_values = np.vstack(all_shap_values)
#
# # 绘制合并后的 SHAP 值图
# plt.figure(figsize=(10, 6))  # 调整图形尺寸
# shap.summary_plot(combined_shap_values, X_train, feature_names=X_train_df.columns[:-1])
#
# plt.title('Combined SHAP Summary Plot for All Classes', fontsize=16, pad=20, fontweight='bold')  # 调整标题的填充距离
...
# plt.savefig(os.path.join(output_dir, 'Combined_SHAP_Summary_Plot_Flam.png'))  # 保存图像到文件
# plt.show()  # 显示图形
...
# 保存最佳模型
joblib.dump(best_model, f'{best_model_name}_best_model_4_F.pkl')
print(f"Best model saved as {best_model_name}_best_model.pkl")

# 将评估结果保存到Excel文件
results_df = pd.DataFrame(results)
results_df.to_excel('model_comparison_results_F.xlsx', index=False)
...
# 加载训练集以获取训练时使用的特征
training_data = pd.read_csv('imputed_selected_features_Flam.csv')

# 提取除去 'SMILES'、'Reactivity' 和 'Classification' 的特征
training_features = training_data.columns.difference(['SMILES', 'Flammability', 'Classification'])
...
best_model = joblib.load('SVM_best_model_4_F.pkl')
...
# 如果需要将预测结果添加到原数据中并保存
hw_list_data['Flam_Prediction'] = hw_reactivity_predictions
hw_list_data.to_excel('HW_list_with_predictions_F.xlsx', index=False)
