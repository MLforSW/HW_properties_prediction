...
# 文件路径列表
file_paths = [
    'SHAP_values_train_Amine_W.csv',
    'SHAP_values_train_Aromatic_W.csv',
    'SHAP_values_train_Ether_W.csv',
    'SHAP_values_train_Halogen_W.csv',
    'SHAP_values_train_Hydroxyl_W.csv',
    'SHAP_values_train_Ketone_Aldehyde_W.csv',
    'SHAP_values_train_Nitrile_W.csv',
    'SHAP_values_train_Other_W.csv',
    'SHAP_values_train_Thioether_W.csv',
    'SHAP_values_train_Thiol_W.csv'
]
...
# 保存合并后的数据到Excel
excel_output_path = os.path.join(output_dir, 'Combined_SHAP_Values_W.xlsx')
feature_means_df.to_excel(excel_output_path, index=True)
...
# 保存前四个特征到Excel
top_features_excel_path = os.path.join(output_dir, 'Top_4_Features_Per_Category_W.xlsx')
...
plt.ylabel('Feature')
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'Average_SHAP_Values_Heatmap_4_W.png'))
plt.close()
...
for label, data in shap_data.items():
    ...
    plt.ylabel('Mean |SHAP Value|')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'SHAP_Feature_Importance_{label}_4_W.png'))
    plt.close()
